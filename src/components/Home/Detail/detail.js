...
const Detail = ({route, navigation}) => {
    ...
    async id => {
      try {
        const result = await dispatch(
          fetchOneTenderData({tenderId: id}),
        ).unwrap();
  ...
  // Clear any stale error left over from a previous (e.g. list) request so it
  // doesn't immediately bounce this screen when opened with passed-in
  // tenderData.
  useEffect(() => {
    dispatch(clearError());
  }, [dispatch]);

  // Effect for handling errors. Only react when this screen actually fetched
  // (no tenderData passed in); otherwise a lingering shared card.error would
  // incorrectly send the user back.
  useEffect(() => {
    if (tenderData || !error) {
      return;
    }
      ...
      <ScrollView
        ...
        <View style={styles.detailCardContainer}>
          <View style={styles.imageContainer}>
            <Image
              source={getTenderImageSource(displayData)}
              style={styles.image}
              resizeMode="contain"
            />
          </View>
          ...
          <View style={styles.detailContainer}>
            ...
            {displayData.last_date_to_apply && (
              <View style={styles.dateContainer}>
                ...
                <Text style={styles.dateText}>
                  <Text style={styles.boldText}>Last Date To Apply: </Text>
                  {displayData.last_date_to_apply}
                </Text>
              </View>
            )}
            ...
            {displayData.days_left && (
              <View style={styles.dateContainer}>
                ...
                <Text style={styles.dateText}>
                  <Text style={styles.boldText}>Days Left: </Text>
                  {displayData.days_left}
                </Text>
              </View>
            )}
