...
const Result = ({navigation}) => {
  ...
  const {data, loading} = useSelector(state => state.result);
  const {dropdowndata} = useSelector(state => state.dropdown);
  const {isAuthenticated} = useSelector(state => state.users);

  // Memoized data
  const organizationData = useMemo(
    () => dropdowndata?.organization_sectors || [],
    [dropdowndata],
  );
  ...
  const projectTypeData = useMemo(
    () => dropdowndata?.project_types || [],
    [dropdowndata],
  );
  const procurementData = useMemo(
    () => dropdowndata?.procurement_types || [],
    [dropdowndata],
  );
  ...
  const handleEndReached = useCallback(() => {
    if (isLoadingMore || !data || page >= data.total_pages) {
      return;
    }
    ...
    async pk => {
      ...
      try {
        ...
        const result = await dispatch(
          fetchOneResultData({tenderId: pk}),
        ).unwrap();
        ...
        <View key={item.pk} style={styles.Card}>
          ...
          <View style={styles.cardContent}>
            ...
            <Text
              ...
              accessibilityLabel={`Public entry name: ${
              ...
              {Array.isArray(item.project_type) &&
                item.project_type.map((project, index) => (
                  <Text
                    ...
                    numberOfLines={2}
                    style={styles.cardText}
                    accessibilityLabel={`Project type ${index + 1}: ${
                      project?.name || 'N/A'
                    }`}>
                    {project?.name || 'N/A'}
                  </Text>
  ...
  const dropdownComponents = useMemo(
    () => [
      ...
      },
      {
        ...
        setState: value =>
          setFilters(prev => ({...prev, procurementsType: value})),
          ...
          <View style={styles.modalContent}>
            ...
            <View style={{padding: 5}}>
              {dropdownComponents.map(
                ({data, placeholder, setState, value, key}) => (
                  <SelectDropdown
                    key={key}
                    data={data || []}
                    onSelect={setState}
                    ...
                    renderSearchInputLeftIcon={() => (
                      <Icon name="search" color={'#424242'} size={20} />
                    )}
